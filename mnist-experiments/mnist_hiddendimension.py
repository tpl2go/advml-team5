...
def data_fraction_experiment():
    ...
    with open('mnist_results_datafrac.pkl', 'wb') as f:
        pickle.dump(results, f)

def hidden_dimension_experiment():
    results = []
    dims = [64, 128, 256, 512, 1024, 2048]
    nb_epochs = 500  # 500
    nb_reps = 3
    K_test = 20 # 20
    l = 1e-2 # NOTE: in appendix D.2
    ...
    for d in dims:
        ...
        for i in range(nb_reps):
            ...
            y = y_train
            Kin = x_train.shape[1]
            print("dimension: {} iter: {} lengthscale: {}".format(d, i, l*Kin**0.5))
            # get model and loss trained for nb_epochs
            model, loss, hist = fit_model_mnist(x, y, nb_epoch=nb_epochs,
                                                batch_size=150, hidden_dim=d,
                                                l=l*Kin**0.5)
            # model.summary()
            accuracies = np.array([model.evaluate(x_test, y_test) for _ in range(K_test)])
            # MC sample due to sampling from concrete dist, Ktest x Ntest x 10
            ...
            rep_results += [(accuracies, ps, epistemic_uncertainty)]
        results += [rep_results]
        with open('mnist_results_dimension.pkl', 'wb') as f:
            print("Saving intermediate results")
            pickle.dump(results, f)
    print("Finished")

if __name__ == '__main__':
    hidden_dimension_experiment()
